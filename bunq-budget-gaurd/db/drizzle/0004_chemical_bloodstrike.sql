    ...
    COALESCE(
      (
        SELECT SUM(
          CAST(
            CASE 
              WHEN (t.json->'NotificationUrl'->'object'->'amount'->>'value') IS NOT NULL 
              AND (t.json->'NotificationUrl'->'object'->'amount'->>'value')::text ~ '^-?[0-9]+(.[0-9]+)?$'
              THEN (t.json->'NotificationUrl'->'object'->'amount'->>'value')::numeric 
              ELSE 0 
            END AS numeric
          )
        )
